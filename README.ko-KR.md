...
소설(txt)을 가져오면 NovelForge가 자동으로 플레이 가능한 비주얼 노벨로 변환합니다. PC(exe)·Android(APK)·웹(zip)으로 내보낼 수 있습니다.
...
| | 설명 |
|---|---|
| 📖 **완전 자동 파이프라인** | ① AI 챕터 분할 → ② 다국어 번역(선택) → ③ 캐릭터/장면/아이템 카드 추출 → ④ 시나리오 작성(자동 CG 연출·아이템 클로즈업·첫 등장 소개 카드·영상 슬롯) → ⑤ 입화/배경/CG/아이템 이미지 생성 → ⑥ TTS 음성 → ⑦ 표준 WebGAL 프로젝트 조립 |
| 🎨 **배경 없는 입화** | AI 세그멘테이션(isnet-anime, 첫 사용 시 모델 자동 다운로드·캐시) 우선, 실패/오프라인 시 개선된 크로마키(연결 flood-fill + 녹색 테두리 제거 + 가장자리 페더링)로 자동 폴백 |
