...
<template>
    <div v-if="projectState.lastResult" class="card">
      <div class="card-head">
        <h3>{{ t("分阶段状态（无意见且未勾全量=只补缺失/失败项，不计费；填意见或勾全量=全量重跑，计费；分章除外：点击即全书重分）") }}</h3>
      ...
      <p class="hint mt-3">
        ...
        {{ t("「重新生成」先只跑该阶段；跑完若下游还有内容没生成（图像/配音），会自动算好规模、确认一次后连续补齐，并重新组装刷新预览（复用缓存只补缺失，组装免费）。分章/翻译/提取属文本上游，仍只跑该阶段——它们重跑后剧本本身要再跑一遍，下游需手动依次重跑。需一次跑通多阶段请切到「整书生成」勾选阶段后开始。") }}
      </p>
    </div>
