...
async function main(): Promise<void> {
  // 1) 可重试错误（429）：失败 4 次后成功（默认 retries=4 → 共 5 次调用）
  let calls = 0;
  const ok = await withRetry(async () => {
    calls++;
    if (calls < 5) throw { status: 429, message: "HTTP 429" };
    return "done";
  }, { delayFor: () => 0 });
  assert(ok === "done" && calls === 5, `应重试后成功，实际调用 ${calls} 次`);
  ...
  // 3) 重试耗尽：仍抛最终错误（默认 retries=4 → 共 5 次调用）
  calls = 0;
  threw = false;
  try {
    await withRetry(async () => {
  ...
  assert(threw && calls === 5, `重试耗尽应失败，实际调用 ${calls} 次`);
