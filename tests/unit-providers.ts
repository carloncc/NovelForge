...
function main(): void {
  ...
  assert(siliconflow?.supports.image === true && siliconflow.supports.tts === true, "硅基流动应支持三通道");

  const channels = { llm: config(), image: config(), tts: config() };
  ...
  assert(channels.image.baseUrl === "" && channels.tts.baseUrl === "", "不支持的通道不应伪造地址");
  ...
  assert(models.length === 3, "应解析 OpenAI 兼容的 data 模型列表");
  assert(models[0].capabilities.includes("llm"), "DeepSeek 模型应识别为文本");
  assert(models[1].capabilities.includes("image"), "FLUX 模型应识别为图片");
  assert(models[2].capabilities.includes("tts"), "CosyVoice 模型应识别为 TTS");
