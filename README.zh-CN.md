...
导入小说 txt，NovelForge 自动将其转换为可游玩的视觉小说——可导出为 PC（exe）、Android（APK）与网页版（zip）。
...
| | 说明 |
|---|---|
| 📖 **全自动管线** | ① AI 分章 → ② 多语翻译（可选）→ ③ 提取角色/场景/物品卡 → ④ 剧本编排（自动 CG 演出/物品特写/登场资料卡/视频推荐位）→ ⑤ 生成立绘/背景/CG/物品图 → ⑥ TTS 配音 → ⑦ 组装标准 WebGAL 项目 |
| 🎨 **无背景立绘** | AI 抠图（isnet-anime 分割，模型首次自动下载并缓存）优先，失败/离线自动回退改进版色度键抠图（连通 flood-fill + 去绿边 + 羽化） |
