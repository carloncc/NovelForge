...
<p>
  ...
  <a href="https://www.forgepeaknow.com/?utm_source=github&utm_medium=readme&utm_content=zh-cn"><img alt="Website" src="https://img.shields.io/badge/Website-forgepeaknow.com-64748b"></a>
</p>
Tauri 2 + Vue 3 + WebGAL · 轻量快速 · API 全自定义

[English](README.md) · **简体中文** · [日本語](README.ja-JP.md) · [한국어](README.ko-KR.md) · [官网](https://www.forgepeaknow.com/?utm_source=github&utm_medium=readme&utm_content=zh-cn) · [更新日志](https://github.com/carloncc/NovelForge/releases)
...
> 需要自备 API key（没有 key？自动进入演示模式，内置示例小说跑通全流程）。启用图像时，批量生成前有一次风格确认（视觉守门）。
...
| 官网 | [https://www.forgepeaknow.com/?utm_source=github&utm_medium=readme&utm_content=zh-cn](https://www.forgepeaknow.com/?utm_source=github&utm_medium=readme&utm_content=zh-cn) —— Web 演示与最新动态 |
...
3. **生成项目**：开关（图像/表情差分/配音/视频位/BGM/登场资料卡）→ 开始生成 → 文本阶段先行；启用图像时先进入**视觉守门确认**（选风格来源 → 生成/确认三视图 → 批准）→ 批准后续跑图像/配音/组装 → 进度日志 + 费用统计。支持分阶段生成（只重跑需要的阶段），AI 分章可提供人工意见反馈
...
| 📖 **视觉守门门禁** | 批量生图前必须确认「视觉守门」：选风格来源（上传参考图由视觉通道分析 / LLM 分析整本小说生成风格示例）+ 每个主角三视图（正/侧/背）。单独重生成某角色三视图不牵连他人；任何输入变化都会使批准失效 |
...
| 🌐 **界面 5 语** | 应用界面自动跟随系统语言（简中 / 繁中 / 英文 / 日文 / 韩文），可随时切换——小说内容与导出游戏的翻译另由「目标语言」控制。 |
...
| 导出 | 视觉守门 |
|---|---|
| ![导出](docs/screenshots/export.png) | ![视觉守门](docs/screenshots/visual-bible-novel_analysis-1280.png) |
...
启用图像生成时，项目必须先确认**视觉守门**才能批量生图（文本提取/剧本不受影响）：

- **风格来源二选一**：上传参考图（视觉通道分析并作为全局风格参考），或让 LLM 分析整本小说后生成一张无角色风格示例图。
- **角色三视图**：每个主角生成正/侧/背三视图，可单独「重新生成三视图」，可上传角色参考图；只影响该角色。
- **确认与批准**：逐角色确认 → 批准视觉守门 → 自动续跑图像/配音/组装。任何小说、角色或风格输入变化都会使已批准状态失效。
- **绝不静默降级**：模型不支持参考图报 `REFERENCE_UNSUPPORTED`，参考文件缺失报 `REFERENCE_MISSING`；视觉通道未配置时给出明确提示。

**存储位置**：全部在项目输出目录 `.novel2vn/visual-bible/`（清单 `visual-bible.json` 只存相对路径/提示词/确认状态，不存 base64；图片为 `style-reference.*`、`style-sample.png`、`threeview_<角色ID>.png` 等）。若提示参考文件缺失，通常是目录被移动或删除：放回原路径或在视觉守门页重新上传/重新生成即可。
...
└─ game/
   ...
   └─ .novel2vn/                # 缓存与中间产物（可删除）
      └─ visual-bible/          # 视觉守门：清单 + 风格参考/示例 + 角色三视图
```
...
- [官网](https://www.forgepeaknow.com/?utm_source=github&utm_medium=readme&utm_content=zh-cn) —— Web 演示与最新动态
...
- [ ] 旁白/独白配音（旁白音色）
- [ ] 图片立绘嘴型同步（差分口型）
- [官网修改方案](./docs/WEBSITE_PLAN.md) —— 产品页 / UTM 漏斗 / 统计闭环
